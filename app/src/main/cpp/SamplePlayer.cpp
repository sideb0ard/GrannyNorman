...
namespace {
    ...
    StereoValue SamplePlayer::Generate(TimingData timing_data) {
        ...
        double left_accumulater, right_accumulater = 0;

        for (int i = 0; i < highest_grain_num_; i++) {

            StereoValue tmp = SoundGrainGenerate(i);
            left_accumulater += tmp.first;
            right_accumulater += tmp.second;
        }

        return std::make_pair(left_accumulater, right_accumulater);
    }
